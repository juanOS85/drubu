...
//    args["markers"] = null;
    var yArgs = getArgs("?" + yourLayers.permalink());
    for (var arg in yArgs) {
      args[arg] = yArgs[arg];
    }
  ...
  if (node) {
    if (zoom >= 13) {
      ...
      node.href = setArgs("http://openstreetmap.org/edit", args);
  ...
  if (node) {
    if (zoom >= 11) {
      ...
      node.href = setArgs("http://www.openstreetmap.org/history", args);
